...
def editing_json(id: int):
        if(os.path.isfile('date.json')):
            with open('data.json') as json_file:
                data = json.load(json_file)
                print("Введите поле, которое Вы будете редактировать")
                field = str(input())
                print("Введите значение, на которое Вы хотите заменить выбранное Вами поле: ")
                for item in data['note']:
                    if item['id'] == id:
                        if (filed == "id"):
                            item[filed] = int(input())
                        else:
                            item[filed] = input()
                        break 
...
def get_base_josn_description(id: int):
        if (os.path.isfile('data.json')):
            with open('data.json', 'r') as json_file:
                data = json.load(json_file)
                for item in data['note']:
